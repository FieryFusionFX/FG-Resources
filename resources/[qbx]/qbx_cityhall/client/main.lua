...
local function openIdentificationMenu(closestCityhall)
    ...
    for license in pairs(availableLicenses) do
        -- id/driver are granted by default, so this menu only offers to reprint the physical
        -- card for those. Weapon isn't granted by default, so it's always shown here instead -
        -- this is where a player actually applies for/buys the permission itself.
        if license ~= 'weapon' and license and not licensesMeta[license] then
            availableLicenses[license] = nil
        end
    end
    local identityOptions = {}
    for item, id in pairsInOrder(availableLicenses) do
        local alreadyLicensed = item == 'weapon' and licensesMeta[item]
        identityOptions[#identityOptions + 1] = {
            title = id.label,
            description = alreadyLicensed and locale('info.already_licensed') or locale('info.price', id.cost),
            disabled = alreadyLicensed or nil,
            onSelect = function()
