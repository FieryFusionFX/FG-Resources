...
-- Maps each seed item name to its sharedConfig.plants key (item names use hyphens, plant keys use underscores)
local seedSortMap = {
    ...
    for i = 1, #tbl do
        ...
        if plyDistance < 0.8 then
            ...
            if plant.health > 0 then
                sleep = 0
                local healthyGrowTag = plant.healthyGrow == 1 and ' ~p~[Healthy Grow]~w~' or ''
                if plant.stage ~= #sharedConfig.plants[plant.sort].stages then
                    local label = ('%s%s~w~ [%s] | %s ~b~%s~w~ | %s ~b~%s~w~%s'):format(locale('text.sort'), sharedConfig.plants[plant.sort].label, gender, locale('text.nutrition'), plant.food, locale('text.health'), plant.health, healthyGrowTag)
                    qbx.drawText3d({text = label, coords = plant.coords})
                else
                    local label = ('%s ~g~%s~w~ [%s] | %s ~b~%s~w~ | %s ~b~%s~w~%s'):format(locale('text.sort'), sharedConfig.plants[plant.sort].label, gender, locale('text.nutrition'), plant.food, locale('text.health'), plant.health, healthyGrowTag)
...
-- Entry point wired to each seed item's `client.export` in ox_inventory's items.lua.
-- Resolves the strain from the item name and forwards to placePlant.
exports('useSeed', function(data, item)
    local sort = seedSortMap[item.name]
    if not sort then
        return exports.qbx_core:Notify(locale('error.cant_place_here'), 'error')
    end

    exports.qbx_weed:placePlant(sort, item)
end)
...
exports('useHealthyGrow', function()
    if closestTarget == 0 then
        return exports.qbx_core:Notify(locale('error.not_safe_here'), 'error')
    end
    ...
    if data.healthyGrow == 1 then
        return exports.qbx_core:Notify(locale('error.already_healthy_grow'), 'error')
    end
    ...
    then
        TriggerServerEvent('qbx_weed:server:applyHealthyGrow', currentProperty, data.id)
    else
        exports.qbx_core:Notify(locale('error.process_canceled'), 'error')
    end
...
RegisterNetEvent('qbx_weed:client:refreshPlantStats', function()
    ...
    if currentProperty then
        ...
        -- Capture known plant ids before despawning wipes the local cache, so we can
        -- re-fetch fresh data instead of respawning from an emptied table (this was
        -- previously making every outside plant disappear on every grow/food tick).
        local knownIds = {}
        for i = 1, #outsidePlants do
            knownIds[#knownIds + 1] = outsidePlants[i].id
        end
        ...
        if #knownIds > 0 then
            outsidePlants = lib.callback.await('qbx_weed:server:getOutsidePlants', false, knownIds)
        end
        spawnOutsidePlants()
    end
end)
