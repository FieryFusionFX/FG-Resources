...
RegisterNetEvent('qbx_weed:server:placePlant', function(coords, sort, property)
    ...
    if not player or player.PlayerData.job.name ~= 'thief' then return end
...
RegisterNetEvent('qbx_weed:server:harvestPlant', function(property, seedAmount, plantItemName, plantId, plantCoords)
    ...
    local player = exports.qbx_core:GetPlayer(source)
    if not player or player.PlayerData.job.name ~= 'thief' or player.PlayerData.metadata.currentPropertyId ~= property or #(GetEntityCoords(GetPlayerPed(player.PlayerData.source)) - plantCoords) > 2 then return end

    local plantHealthyGrow = MySQL.prepare.await('SELECT healthyGrow FROM weed_plants WHERE id = ?', { plantId })
    if plantHealthyGrow == nil then
    ...
    if plantHealthyGrow == 1 then
        harvestAmount = math.ceil(harvestAmount * config.healthyGrowYieldMultiplier)
    end
...
---@param property string
---@param plantId integer
RegisterNetEvent('qbx_weed:server:applyHealthyGrow', function(property, plantId)
    if (property and sharedConfig.plantsSpawnType == 'outside') or (not property and sharedConfig.plantsSpawnType == 'property') then return end

    local player = exports.qbx_core:GetPlayer(source)
    if not player or player.PlayerData.job.name ~= 'thief' then return end

    if not exports.ox_inventory:RemoveItem(player.PlayerData.source, sharedConfig.items.healthyGrow, 1) then return end

    MySQL.update.await('UPDATE weed_plants SET food = 100, health = 100, healthyGrow = 1 WHERE id = ?', { plantId })
    exports.qbx_core:Notify(player.PlayerData.source, locale('text.healthy_grow_applied'), 'success')
