...
local function sendStatusMessage(statusList)
    ...
    local templateMiddle = '<strong>'.. config.partLabels.engine ..' (engine):</strong> {1} <br><strong>'.. config.partLabels.body ..' (body):</strong> {2} <br><strong>'.. config.partLabels.radiator ..' (radiator):</strong> {3} <br><strong>'.. config.partLabels.axle ..' (axle):</strong> {4}<br><strong>'.. config.partLabels.brakes ..' (brakes):</strong> {5}<br><strong>'.. config.partLabels.clutch ..' (clutch):</strong> {6}<br><strong>'.. config.partLabels.fuel ..' (fuel):</strong> {7}<br><strong>'.. config.partLabels.tire ..' (tire):</strong> {8}'
    TriggerEvent('chat:addMessage', {
        template = templateStart .. templateMiddle .. templateEnd,
        args = {locale('labels.veh_status'),
            qbx.math.round(statusList.engine) .. "/" .. sharedConfig.maxStatusValues.engine .. " ("..exports.ox_inventory:Items()[sharedConfig.repairCost.engine].label..")",
            ...
            qbx.math.round(statusList.tire) .. "/" .. sharedConfig.maxStatusValues.tire .. ".0 ("..exports.ox_inventory:Items()[sharedConfig.repairCost.tire].label..")"
        }
    })
end
...
local function repairPart(part)
    ...
    if lib.progressBar({
        ...
        elseif part == "tire" then
            ...
            TriggerServerEvent("vehiclemod:server:updatePart", plate, "tire", sharedConfig.maxStatusValues[part])
        else
