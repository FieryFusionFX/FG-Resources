...
-- Higher tire wear = higher chance a bad tire actually blows out, mirroring the other parts' escalating effects
local function applyTireEffects(vehicle, plate)
    local tireHealth = VehicleStatus[plate].tire
    local burstChance
    if tireHealth <= 80 and tireHealth >= 60 then
        burstChance = 5
    elseif tireHealth <= 59 and tireHealth >= 40 then
        burstChance = 15
    elseif tireHealth <= 39 and tireHealth >= 20 then
        burstChance = 30
    elseif tireHealth <= 19 and tireHealth >= 6 then
        burstChance = 50
    else
        burstChance = 75
    end

    if math.random(1, 100) <= burstChance then
...
local function applyEffects(vehicle)
    ...
    if VehicleStatus[plate].radiator <= 80 and (chance >= 1 and chance <= 16) then
        applyRadiatorEffects(vehicle, plate)

    elseif VehicleStatus[plate].axle <= 80 and (chance >= 17 and chance <= 33) then
        applyAxleEffects(vehicle, plate)

    elseif VehicleStatus[plate].brakes <= 80 and (chance >= 34 and chance <= 50) then
        applyBrakeEffects(vehicle, plate)

    elseif VehicleStatus[plate].clutch <= 80 and (chance >= 51 and chance <= 67) then
        applyClutchEffects(vehicle, plate)

    elseif VehicleStatus[plate].fuel <= 80 and (chance >= 68 and chance <= 84) then
        leakFuel(vehicle, plate)

    elseif VehicleStatus[plate].tire <= 80 and (chance >= 85 and chance <= 100) then
        applyTireEffects(vehicle, plate)
    end
end
