...
-- Wear/status persistence: player_vehicles.status was previously never actually written back to,
-- so wear reset on every restart. dirtyPlates tracks what's changed since the last flush.
local dirtyPlates = {}

local function persistVehicleStatus(plate)
...
CreateThread(function()
    while true do
        ...
        for plate in pairs(dirtyPlates) do
            persistVehicleStatus(plate)
        end
        dirtyPlates = {}
    end
end)

-- Flush immediately whenever qbx_vehicles saves this vehicle for any reason (garage store/retrieve, phone call-in, etc.)
AddEventHandler('qbx_vehicles:server:vehicleSaved', function(vehicleId)
    ...
    if not vehicleData or not vehicleData.props or not vehicleData.props.plate then return end
    ...
    local statusInfo = vehicleStatus[plate] or getVehicleStatus(plate) or
        {
            ...
            tire = sharedConfig.maxStatusValues.tire
        }
    -- Guards against status JSON saved before tire wear was added
    statusInfo.tire = statusInfo.tire or sharedConfig.maxStatusValues.tire
