...
---Garages every vehicle a citizen currently has OUT and spawned in the world.
---Used on disconnect/logout so a car left sitting around doesn't stay in the world unattended.
---@param citizenid string
local function storeOutVehicles(citizenid)
    local outVehicles = exports.qbx_vehicles:GetPlayerVehicles({ citizenid = citizenid, states = VehicleState.OUT })
    if not outVehicles then return end

    for i = 1, #outVehicles do
        local vehicle = findVehicleEntityByPlate(outVehicles[i].props.plate)
        if vehicle then
            exports.qbx_vehicles:SaveVehicle(vehicle, { state = VehicleState.GARAGED })
            exports.qbx_core:DeleteVehicle(vehicle)
        end
    end
end
...
---@param source number
local function onPlayerGone(source)
    local player = exports.qbx_core:GetPlayer(source)
    if not player then return end
    storeOutVehicles(player.PlayerData.citizenid)
end

-- Covers both an outright disconnect and a manual character-switch logout, mirroring the
-- pattern qbx_vehiclekeys uses for the same pair of events.
AddEventHandler('playerDropped', function()
    onPlayerGone(source)
end)

RegisterNetEvent('QBCore:Server:OnPlayerUnload', onPlayerGone)