...
RegisterNUICallback("npwd:qbx_garage:requestWaypoint", function(data, cb)
    local content
    if findVehFromPlateAndLocate(data.plate) then
        content = locale('notification.marked')
    else
        -- Not currently spawned in the world - try to call it in from storage instead
        local result = lib.callback.await('npwd_qbx_garages:server:callVehicle', false, data.plate)
